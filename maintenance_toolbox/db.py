...
class Planning(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="draft")  # draft / validated / rex_pending / closed / archived
    ...
    tasks: Mapped[list["PlanningTask"]] = relationship(back_populates="planning", cascade="all, delete-orphan")
    teams: Mapped[list["PlanningTeam"]] = relationship(back_populates="planning", cascade="all, delete-orphan")
...
class PlanningTask(Base):
    ...
    task_type: Mapped[str] = mapped_column(String(32), default="ot")  # ot/manual
    ...
    operation_mode: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # start/end/free for manual
...
def make_engine(echo: bool = False):
    ...
    if not database_url:
        ...
        db_path = os.path.join(os.path.dirname(__file__), "maintenance_toolbox.db")
...
def ensure_org_defaults(session: Session, org: Organization) -> None:
    mapping = session.scalar(select(FieldMapping).where(FieldMapping.organization_id == org.id))
    if mapping is None:
        mapping = FieldMapping(organization_id=org.id)
        session.add(mapping)
    existing = session.scalars(select(RexCause).where(RexCause.organization_id == org.id)).all()
    if not existing:
        for fr, en, nl in DEFAULT_CAUSES:
            session.add(RexCause(
...
def log_action(session: Session, user_id: Optional[int], organization_id: Optional[int], action: str, details: str = "") -> None:
    session.add(AuditLog(user_id=user_id, organization_id=organization_id, action=action, details=details))
    session.commit()
